        ...
        for which in ["mfcc", "msd", "openl3"]:
            ...
            for _ in range(repetitions):
                ...
                for train_index, test_index in kf.split(X):
                    ...
                    model.eval()
                    with torch.no_grad():
                        y_reg_pred, y_cls_pred = model(torch.from_numpy(X_test).float())
            ...
            print(
                f"n_tar_cls: {n_tar_cls}, modality: {modality}, which: {which}, voice: {voice}, repetitions: {repetitions}"
            )
            ...
            filename = f"new_results_{n_tar_cls}_{modality}_{which}_{voice}.txt"
