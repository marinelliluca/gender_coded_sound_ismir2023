...
for n_classes in [2,3]:
    for voice in [True, False]:
        for which in ["mfcc", "msd", "openl3"]:
            ...
            filename = f"results_{n_classes}_{modality}_{which}_{voice}.txt"
            with open(filename, "a") as f:
                f.write(f"n_classes: {n_classes}, modality: {modality}, which: {which}, voice: {voice}, repetitions: {repetitions}\n")
                f.write(f"\tF1: {np.mean(all_f1s):.2f} ± {np.mean(np.std(all_f1s, axis=1)):.2f}\n")
                f.write("\tR2:\n")
                for i, response in enumerate(emotions_and_mid_level.columns):
                    f.write(f"\t\t{response}: {np.mean(all_r2s[:, :, i]):.2f} ± {np.std(all_r2s[:, :, i]):.2f}\n")
                f.write("\tPearson's r:\n")
                for i, response in enumerate(emotions_and_mid_level.columns):
                    f.write(f"\t\t{response}: {np.mean(all_pearsons[:, :, i]):.2f} ± {np.std(all_pearsons[:, :, i]):.2f}\n")
                f.write(f"Average R2 for emotion responses: {mean_emo_r2:.2f} ± {std_emo_r2:.2f}\n")
                f.write(f"Average Pearson's r for emotion responses: {mean_emo_pears:.2f} ± {std_emo_pears:.2f}\n")
                f.write(f"Average R2 for mid-level responses: {mean_mid_r2:.2f} ± {std_mid_r2:.2f}\n")
                f.write(f"Average Pearson's r for mid-level responses: {mean_mid_pears:.2f} ± {std_mid_pears:.2f}\n")
