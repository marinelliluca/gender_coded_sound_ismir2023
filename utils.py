...
# TODO:
#   - split these functions into different scripts
...
def load_embeddings_and_labels(
    ...
    y_reg = np.empty((groundtruth_df.shape[0], emotions_and_mid_level_df.shape[1]))
...
def results_to_dict(
    ...
    for i, response in enumerate(mid_cols + emo_cols):
        if i < len(mid_cols):
            ...
            is_significant = multipletests(
                all_ps_emo[:, :, i - len(mid_cols)].flatten(),
                alpha=0.05,
                method="holm-sidak",
            )[0]
            rat_sig = np.sum(is_significant) / len(is_significant)
...
def display_results(dictionary):
            ...
            for subkey, subvalue in value.items():
                if isinstance(subvalue, dict):
                    for subsubkey, subsubvalue in subvalue.items():
                        target_rows.append(
                            [key, subkey, subsubkey, format_value(subsubvalue)]
                        )
                        key = ""
                        subkey = ""
                else:
    ...
    average_rows.append(
        ["", "secondary f1", "mean", format_value(np.mean(f1_secondary_means))]
    )
        ...
        if isinstance(value, dict):
            for subkey, subvalue in value.items():
                if isinstance(subvalue, dict):
                    for subsubkey, subsubvalue in subvalue.items():
                        rows.append(
                            [key, subkey, subsubkey, type(subsubvalue).__name__]
                        )
                        key = ""
                        subkey = ""
                else:
...
def experiments_to_dict_of_dfs(folder, rep, fold):
    ...
    dict_of_dfs = {tt: pd.DataFrame(columns=columns) for tt in target_task}
    ...
    for fn in glob.glob(f"./{folder}/*_rep_{rep}_fold_{fold}.json"):
        ...
        f1_secondary_means = [
            v["mean"] for k, v in results["f1"].items() if k != "target"
        ]
        f1_secondary_stds = [
            v["std"] for k, v in results["f1"].items() if k != "target"
        ]
        results["average"]["secondary f1"] = {
            "mean": np.mean(f1_secondary_means),
            "std": np.mean(f1_secondary_stds),
        }
        ...
        tt = (
            "Binary"
            if len(results["config"]["classifications"]["target"]) == 2
            else "Ternary"
        )

        # Add row to the appropriate DataFrame
        df = dict_of_dfs[tt].copy()
        dict_of_dfs[tt] = pd.concat(
...
class DynamicMultitasker(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_cls = sum(
            [
                F.cross_entropy(y_hat_cls[k], v, ignore_index=-1)
                for k, v in y_cls.items()
            ]
        )
